...
function draw() {
 ...
 fill(190);
  ...
  for (var x = 0; x <= width; x += 10) {
    ...
    // Option #1: 2D Noise
    var y = map(noise(xoff, yoff), 0, 1, 200,280);
  ...
  for (var x = 0; x <= width; x += 10) {
    ...
    // Option #1: 2D Noise
    var y = map(noise(xoff2, yoff2), 0, 1, 250,320);
  ...
  for (var x = 0; x <= width; x += 10) {
    ...
    // Option #1: 2D Noise
    var y = map(noise(xoff3, yoff3), 0, 1, 300,380);
  ...
  endShape(CLOSE);
  	...
  	// Solo usar cuando esté listo para exportar
	//if(frameCount<=70){
	//saveCanvas("Viaje" + frameCount, 'png');
	//}
